...
try {
  console.log("try to load: " + process.env.PUBLIC_KEY_FILE);
  exports.pubKey = JSON.parse(fs.readFileSync(process.env['PUBLIC_KEY_FILE']));
  console.log("try to load: " + process.env.SECRET_KEY_FILE);
  _privKey = JSON.parse(fs.readFileSync(process.env['PRIVATE_KEY_FILE']));
} catch(e) { 
